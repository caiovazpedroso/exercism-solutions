...
def approx_average_is_average(hand):
    ...
    """
    if card_average(hand) == statistics.median(hand):
        return True
...
def average_even_is_average_odd(hand):
    ...
    while index < len(hand):
        if hand[index] % 2 == 0:
            even_hand.append(hand[index])
        odd_hand.append(hand[index])
        index += 1
    return bool(card_average(even_hand) == card_average(odd_hand))
